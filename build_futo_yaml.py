...
# Which moreKey leads, where the canonical rule picks the wrong one. Leading
# also means hinting, so this is the letter printed in the key's corner.
LEAD_MOREKEY: dict[str, str] = {
    "\u0438": "\u0457",   # и -> hint ї, not і
}
...
#
# `fallbackKey` takes the scalar shortcut rather than a nested map. BaseKey's
# deserializer turns a bare scalar into BaseKey(spec), so the two parse to the
# same object, and the scalar is what LayoutSpec.md prints for the default
# bottom row. The nested form is only needed when the fallback carries its own
# attributes, which is why Default/clearflow.yaml and Default/kasroz.yaml use it
# and nothing else does.
CONTEXTUAL_COMMA_KEY = '{type: contextual, fallbackKey: ","}'
...
def emit(layout_key: str, layers: dict[str, Any], placement: str, strict: bool,
    ...
    L: list[str] = []
    # No version stamp and no generator filename. Both were here and both were
    # wrong to ship: a version this repo cannot bump goes stale on the first
    # in-tree edit, and naming a script that lives in another repository implies
    # hand-edits here will be overwritten. The attribution line stays, in the
    # shape ipa_qwerty.yaml already uses for its credit.
    L.append(f"# {cfg['name']}")
    L.append("# Contributed by Andrei Kirkouski; notes and the desktop layouts")
    L.append("# this follows are at https://polish-typographic.com")
    L.append("")
    ...
    for row_idx, physical in enumerate(PHYSICAL_ROWS):
        ...
        if row_idx == len(PHYSICAL_ROWS) - 1:
            L.append("      - $shift")
        for k in keys:
            diac = list(diacritics.get(k, []))
            lead = LEAD_MOREKEY.get(base[k])
            if lead is not None and lead in diac:
                diac.remove(lead)
                diac.insert(0, lead)
            more = list(diac)
            ...
            line = emit_letter_key(k, base[k], more, strict,
                                   hint=diac[0] if diac else (aps[0] if aps else None))
            # A moreKey that repeats its own key's spec reads as a copy-paste
            # error to anyone opening the file, since the reason lives in the
            # engine rather than on the page. Say it on the line.
            if keep is not None and keep in more:
                line += (f" # keeps {keep} reachable where KeySpecShortcuts"
                         " replaces this keycap; dedup drops it elsewhere")
            L.append(line)
        if row_idx == len(PHYSICAL_ROWS) - 1:
            L.append("      - $delete")
    ...
    L.append("      letters:")
    # Alt-page rows nest their items one level below `letters:`, the way the
    # main `rows:` block already does. The file used to do both -- nested under
    # rows:, flush under altPages: -- and no stock layout mixes the two.
...
def validate(layout_key: str, text: str, layers: dict[str, Any],
    ...
    for key_id, more in collect_diacritics(layers, placement).items():
        ...
        if more:
            # LEAD_MOREKEY overrides which one leads where the canonical rule
            # picks the wrong letter; honour it rather than flagging the override.
            lead = LEAD_MOREKEY.get(layers["base"].get(key_id, ""))
            expect = lead if (lead is not None and lead in more) else more[0]
            want = f'moreKeys: [{escape_unicode(expect)}'
            if want not in text:
                errs.append(
                    f"{expect!r} should lead moreKeys on key {key_id} so it becomes the hint"
                )
